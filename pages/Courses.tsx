...
const CoursesPage: React.FC<CoursesProps> = ({ courses, students, onAddCourse }) => {
  ...
  const [coachName, setCoachName] = useState('李教练');
  const [capacity, setCapacity] = useState(12);
  const [selectedStudentIds, setSelectedStudentIds] = useState<string[]>([]);
  const [studentSearch, setStudentSearch] = useState('');

  const getCourseAt = (day: string, timeKey: string) => {
    return courses.find(c => c.schedule.includes(day) && c.id.includes(timeKey));
  };
                    ...
                    {filteredStudents.length > 0 ? (
                      filteredStudents.map(student => (
                        <button 
                          ...
                          className={`flex items-center gap-4 p-4 rounded-[1.5rem] border-2 transition-all text-left relative group/card ${
